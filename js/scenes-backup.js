...
(function(){
  ...
  const bg=new THREE.Mesh(new THREE.PlaneGeometry(2,2),new THREE.ShaderMaterial({
    ...
    fragmentShader:`
      ...
      void main(){
        ...
        float d=length(c);
        float ns=n(c*3.+uT*.08)*n(c*6.-uT*.05)*n(c*1.5+uT*.12);
        vec3 col=mix(vec3(.08,.04,.12),mix(vec3(.32,.14,.04),vec3(.54,.38,.12),smoothstep(.2,.7,ns)),smoothstep(.5,.0,d+ns*.3));
        col*=1.-d*.6;gl_FragColor=vec4(col,1.);
      }
  ...
  function mL(cnt,sz,ca,cb,sp){
    ...
    const gr=ctx.createRadialGradient(32,32,0,32,32,32);gr.addColorStop(0,'rgba(255,255,255,1)');gr.addColorStop(.4,'rgba(255,255,255,.5)');gr.addColorStop(1,'rgba(255,255,255,0)');ctx.fillStyle=gr;ctx.fillRect(0,0,64,64);
  ...
  const L1=mL(3500,.055,0xc9a14a,0x8b3a10,4),L2=mL(1500,.04,0xf2e8d0,0xc9a14a,5),L3=mL(600,.08,0xc4501c,0xc9a14a,2.5);
  scene.add(L1,L2,L3);
  for(let i=0;i<5;i++){const cc=document.createElement('canvas');cc.width=cc.height=256;const ct=cc.getContext('2d');const cg=ct.createRadialGradient(128,128,0,128,128,128);const col=i%2===0?'rgba(139,58,16,':'rgba(201,161,74,';cg.addColorStop(0,col+'.1)');cg.addColorStop(1,col+'0)');ct.fillStyle=cg;ct.fillRect(0,0,256,256);const sp=new THREE.Sprite(new THREE.SpriteMaterial({map:new THREE.CanvasTexture(cc),transparent:true,blending:THREE.AdditiveBlending,depthWrite:false}));sp.scale.set(6+Math.random()*4,4+Math.random()*3,1);sp.position.set((Math.random()-.5)*8,(Math.random()-.5)*5,(Math.random()-.5)*2-3);scene.add(sp);}
  let t=0;
  (function a(){t+=.005;bg.material.uniforms.uT.value=t;L1.rotation.y=t*.04;L1.rotation.x=Math.sin(t*.05)*.08;L2.rotation.y=-t*.03;L3.rotation.y=t*.07;
    cam.position.x+=((MX/innerWidth-.5)*.5-cam.position.x)*.03;
    cam.position.y+=((MY/innerHeight-.5)*-.35-cam.position.y)*.03;
    cam.lookAt(0,0,0);requestAnimationFrame(a);rend.render(scene,cam);})();
  window.addEventListener('resize',()=>{rend.setSize(W(),H());cam.aspect=W()/H();cam.updateProjectionMatrix();bg.material.uniforms.uR.value.set(W(),H());});
})();
...
(function(){
  ...
  const W=()=>P.offsetWidth,H=()=>P.offsetHeight;
  const rend=mkR(cv,false);rend.setSize(W(),H());rend.setClearColor(0x06040e,1);
  ...
  const vsl=new THREE.Mesh(vG,new THREE.MeshStandardMaterial({color:0x4a1c08,roughness:.75,metalness:.05,side:THREE.DoubleSide}));
  vsl.scale.setScalar(.6);scene.add(vsl);
  const vW=new THREE.Mesh(vG,new THREE.MeshBasicMaterial({color:0xc9a14a,wireframe:true,transparent:true,opacity:.12}));
  vW.scale.setScalar(.61);scene.add(vW);
  [-.5,.4,.9,1.4].forEach(y=>{const r=new THREE.Mesh(new THREE.TorusGeometry(.84,.016,8,64),new THREE.MeshStandardMaterial({color:0xc9a14a,metalness:.8,roughness:.2}));r.scale.setScalar(.6);r.position.y=y*.6;r.rotation.x=Math.PI/2;scene.add(r);});
  const lG=new THREE.CylinderGeometry(.75,.65,.12,64);
  const liq=new THREE.Mesh(lG,new THREE.MeshStandardMaterial({color:0xd4a030,transparent:true,opacity:.9,metalness:.2,roughness:.05,emissive:0x8b4000,emissiveIntensity:.4}));
  ...
  const bubs=new THREE.Points(bG,new THREE.PointsMaterial({color:0xf0c060,size:.03,transparent:true,opacity:.7,blending:THREE.AdditiveBlending}));
  bubs.scale.setScalar(.6);scene.add(bubs);
  const oR=new THREE.Mesh(new THREE.TorusGeometry(2.4,.02,4,80),new THREE.MeshBasicMaterial({color:0xc9a14a,transparent:true,opacity:.2}));
  ...
  scene.add(new THREE.Points(hG,new THREE.PointsMaterial({color:0xc9a14a,size:.022,transparent:true,opacity:.45,blending:THREE.AdditiveBlending})));
  let t=0;const lOrig=lG.attributes.position.array.slice();
  (function a(){t+=.007;vsl.rotation.y=t*.45;vW.rotation.y=t*.45;liq.rotation.y=t*1.5;oR.rotation.z=t*.2;
    ...
    requestAnimationFrame(a);rend.render(scene,cam);})();
  window.addEventListener('resize',()=>{rend.setSize(W(),H());cam.aspect=W()/H();cam.updateProjectionMatrix();});
})();

// ════════════════════════════════════════
// MENU CARDS — 6 Product Scenes
// ════════════════════════════════════════
[{id:'mc1',bg:0x0a0608,c:0x7a2808,a:0xc9a14a,tp:'vessel'},
 {id:'mc2',bg:0x08060e,c:0x4a0a70,a:0xd0a0ff,tp:'torus'},
 {id:'mc3',bg:0x09080a,c:0x5a4a0a,a:0xf0d060,tp:'icosa'},
 {id:'mc4',bg:0x0a0606,c:0x7a1818,a:0xff7050,tp:'sphere'},
 {id:'mc5',bg:0x060a08,c:0x0c4a20,a:0x60e890,tp:'octa'},
 {id:'mc6',bg:0x0a0800,c:0x4a2a00,a:0xf0b830,tp:'knot'}
  ...
  (function lp(){t+=.012;mesh.rotation.y=t*.55;mesh.rotation.x=Math.sin(t*.3)*.4;wire.rotation.y=t*.55;wire.rotation.x=Math.sin(t*.3)*.4;mesh.position.y=Math.sin(t*.5)*.1;pl.position.set(2*Math.sin(t),2*Math.cos(t*.7),2);requestAnimationFrame(lp);rend.render(scene,cam);})();
});
...
(function(){
  ...
  const mat=new THREE.ShaderMaterial({
    ...
    fragmentShader:`
      ...
      void main(){
        float e=(vE+.32)/.64;
        vec3 col=mix(vec3(.05,.03,.08),mix(vec3(.54,.22,.05),vec3(.79,.63,.29),e),smoothstep(.05,.95,e));
        col+=sin(vUv.y*80.+uT*2.)*.025*vec3(.3,.15,0.);
        vec2 cm=vUv-uM;col+=vec3(.8,.5,.1)*exp(-length(cm)*4.)*.15;
  ...
  const orbs=[];for(let i=0;i<12;i++){const o=new THREE.Mesh(new THREE.SphereGeometry(.08+Math.random()*.1,12,12),new THREE.MeshBasicMaterial({color:0xc9a14a,transparent:true,opacity:.1,blending:THREE.AdditiveBlending}));o.position.set((Math.random()-.5)*7,(Math.random()-.5)*2,Math.random()*-1);o.userData={vx:(Math.random()-.5)*.003,vy:(Math.random()-.5)*.004};scene.add(o);orbs.push(o);}
  let t=0;
  (function a(){t+=.005;mat.uniforms.uT.value=t;mat.uniforms.uM.value.lerp(new THREE.Vector2(MX/innerWidth,1-MY/innerHeight),.04);orbs.forEach(o=>{o.position.x+=o.userData.vx;o.position.y+=o.userData.vy;if(Math.abs(o.position.x)>3.5)o.userData.vx*=-1;if(Math.abs(o.position.y)>1.5)o.userData.vy*=-1;});
    cam.position.x+=((MX/innerWidth-.5)*.3-cam.position.x)*.03;cam.position.y+=((MY/innerHeight-.5)*-.2-cam.position.y)*.03;cam.lookAt(0,0,0);requestAnimationFrame(a);rend.render(scene,cam);})();
  window.addEventListener('resize',()=>{rend.setSize(innerWidth,innerHeight);cam.aspect=innerWidth/innerHeight;cam.updateProjectionMatrix();});
})();
...
(function(){
  ...
  const W=()=>P.offsetWidth,H=()=>P.offsetHeight;
  const rend=mkR(cv,false);rend.setSize(W(),H());rend.setClearColor(0x08060f,1);
  const scene=new THREE.Scene(),cam=mkC(50,W(),H(),6);lgts(scene,0xe8c46a,0x8b3a10);
  const iG=new THREE.IcosahedronGeometry(1.5,2);
  const ctr=new THREE.Mesh(iG,new THREE.MeshStandardMaterial({color:0x6a2808,roughness:.8,metalness:.05,transparent:true,opacity:.9}));scene.add(ctr);
  const cW=new THREE.Mesh(iG,new THREE.MeshBasicMaterial({color:0xc9a14a,wireframe:true,transparent:true,opacity:.15}));cW.scale.setScalar(1.02);scene.add(cW);
  const orb1=new THREE.Mesh(new THREE.TorusGeometry(2.4,.04,4,80),new THREE.MeshBasicMaterial({color:0xc9a14a,transparent:true,opacity:.3}));orb1.rotation.x=.5;scene.add(orb1);
  const orb2=new THREE.Mesh(new THREE.TorusGeometry(3,.02,4,80),new THREE.MeshBasicMaterial({color:0x8b3a10,transparent:true,opacity:.2}));orb2.rotation.x=1.2;orb2.rotation.y=.4;scene.add(orb2);
  ...
  pG.setAttribute('position',new THREE.Float32BufferAttribute(pp,3));
  scene.add(new THREE.Points(pG,new THREE.PointsMaterial({color:0xc9a14a,size:.03,transparent:true,opacity:.5})));
  ...
  hG.setAttribute('position',new THREE.Float32BufferAttribute(hP,3));scene.add(new THREE.Points(hG,new THREE.PointsMaterial({color:0xc9a14a,size:.018,transparent:true,opacity:.3,blending:THREE.AdditiveBlending})));
  let t=0;
  (function a(){t+=.006;ctr.rotation.y=t*.3;ctr.rotation.x=Math.sin(t*.2)*.15;cW.rotation.y=t*.3;cW.rotation.x=Math.sin(t*.2)*.15;orb1.rotation.y=t*.25;orb1.rotation.z=t*.1;orb2.rotation.x=t*.15;orb2.rotation.z=-t*.08;cam.position.x+=((MX/innerWidth-.5)*.5-cam.position.x)*.02;cam.position.y+=((MY/innerHeight-.5)*-.4-cam.position.y)*.02;cam.lookAt(0,0,0);requestAnimationFrame(a);rend.render(scene,cam);})();
  window.addEventListener('resize',()=>{rend.setSize(W(),H());cam.aspect=W()/H();cam.updateProjectionMatrix();});
})();

// ════════════════════════════════════════
// GALLERY SCENES — 7 Items
// ════════════════════════════════════════
[{id:'g1',c:0x7a2208,a:0xc9a14a,s:'vessel',bg:0x080305},{id:'g2',c:0x3a1a05,a:0xe8c030,s:'cube',bg:0x090600},{id:'g3',c:0x5a0a1a,a:0xe06040,s:'torus',bg:0x0a0408},{id:'g4',c:0x1a0a2a,a:0xa090f0,s:'diamond',bg:0x07060e},{id:'g5',c:0x0a2a1a,a:0x60d890,s:'helix',bg:0x060a08},{id:'g6',c:0x2a1808,a:0xf0b030,s:'ring',bg:0x0a0806},{id:'g7',c:0x1a0a08,a:0xd48030,s:'icosa',bg:0x090604}
].forEach(({id,c,a,s,bg})=>{
  const cv=$(('#'+id));if(!cv)return;
  const W=cv.parentElement.offsetWidth||200,H=cv.parentElement.offsetHeight||200;
  const rend=mkR(cv,false);rend.setSize(W,H);rend.setClearColor(bg,1);
  const scene=new THREE.Scene(),cam=mkC(50,W,H,4);
  scene.add(new THREE.AmbientLight(0x111111,.3));const pl=new THREE.PointLight(a,5,10);pl.position.set(2,2,2);scene.add(pl);const pl2g=new THREE.PointLight(c,3,8);pl2g.position.set(-2,-1,0);scene.add(pl2g);
  ...
  let t=Math.random()*100;
  (function lp(){t+=.01;if(mesh.rotation){mesh.rotation.y=t*.45;mesh.rotation.x=Math.sin(t*.3)*.3;}if(wire.rotation){wire.rotation.y=t*.45;wire.rotation.x=Math.sin(t*.3)*.3;}pl.position.set(2*Math.sin(t),2*Math.cos(t*.7),2);requestAnimationFrame(lp);rend.render(scene,cam);})();
});
...
(function(){
  ...
  // Dramatic lighting
  scene.add(new THREE.AmbientLight(0x1a0d00,.6));
  const key=new THREE.DirectionalLight(0xf0b830,3.5);key.position.set(3,5,4);scene.add(key);
  const rim=new THREE.PointLight(0xc9a14a,6,15);rim.position.set(-4,2,-2);scene.add(rim);
  const accent=new THREE.PointLight(0xc4501c,4,10);accent.position.set(2,-3,3);scene.add(accent);
  const topL=new THREE.PointLight(0xf2e8d0,3,8);topL.position.set(0,6,0);scene.add(topL);

  // Premium ceremonial vessel (golden qero)
  ...
  const vG=new THREE.LatheGeometry(vPts,96);
  const vMat=new THREE.MeshStandardMaterial({color:0x2a1800,roughness:.65,metalness:.15,side:THREE.DoubleSide});
  const vsl=new THREE.Mesh(vG,vMat);vsl.scale.setScalar(.55);scene.add(vsl);

  const vW=new THREE.Mesh(vG,new THREE.MeshBasicMaterial({color:0xf0b830,wireframe:true,transparent:true,opacity:.08}));
  ...
  [-.4,.2,.7,1.2,1.7].forEach((y,i)=>{
    const r=new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({color:0xf0b830,metalness:.9,roughness:.1,emissive:0xf0b830,emissiveIntensity:.15})
    );
    r.rotation.x=Math.PI/2;r.position.y=y*.55;r.scale.setScalar(.55);scene.add(r);
  });

  // Inca geometric pattern
  for(let i=0;i<24;i++){
    const ang=(i/24)*Math.PI*2;
    const dot=new THREE.Mesh(
      new THREE.OctahedronGeometry(.03,0),
      new THREE.MeshStandardMaterial({color:0xf0b830,metalness:.95,roughness:.05,emissive:0xc9a14a,emissiveIntensity:.3})
    );
  ...
  const liq=new THREE.Mesh(lG,new THREE.MeshStandardMaterial({
    color:0xf0a000,transparent:true,opacity:.94,metalness:.2,roughness:.02,
    emissive:0xa06000,emissiveIntensity:.4
  }));
  ...
  const steamP=new THREE.Points(sG,new THREE.PointsMaterial({color:0xf2e8d0,size:.04,transparent:true,opacity:.25,blending:THREE.AdditiveBlending,depthWrite:false}));
  ...
  pG.setAttribute('position',new THREE.Float32BufferAttribute(pP,3));
  scene.add(new THREE.Points(pG,new THREE.PointsMaterial({color:0xf0b830,size:.02,transparent:true,opacity:.4,blending:THREE.AdditiveBlending,depthWrite:false})));

  // Orbiting rings
  const oR=new THREE.Mesh(new THREE.TorusGeometry(2,.015,4,80),new THREE.MeshBasicMaterial({color:0xf0b830,transparent:true,opacity:.2}));
  oR.rotation.x=.4;scene.add(oR);
  const oR2=new THREE.Mesh(new THREE.TorusGeometry(2.5,.01,4,80),new THREE.MeshBasicMaterial({color:0xc9a14a,transparent:true,opacity:.1}));
...
(function(){
  ...
  // ── LIGHTS ──
  const ambLight = new THREE.AmbientLight(0x221100, 0.4);
  scene.add(ambLight);
  const keyLight = new THREE.DirectionalLight(0xd4a847, 3);
  keyLight.position.set(4, 6, 4);
  keyLight.castShadow = true;
  scene.add(keyLight);
  const rimLight = new THREE.PointLight(0xc9a14a, 5, 15);
  rimLight.position.set(-4, 2, -2);
  scene.add(rimLight);
  const fillLight = new THREE.PointLight(0x8b3a10, 3, 10);
  fillLight.position.set(2, -3, 3);
  scene.add(fillLight);
  const topLight = new THREE.PointLight(0xf2e8d0, 2, 8);
  ...
  const vesselColors = [
    { body: 0x5c2008, engrave: 0xc9a14a, liquid: 0xd4880a, liquidEmit: 0x8b4000 },
    { body: 0x3a0d5a, engrave: 0xd0a0ff, liquid: 0x6a1a90, liquidEmit: 0x4a0070 },
    { body: 0x4a3808, engrave: 0xf0d060, liquid: 0xd4b020, liquidEmit: 0x906000 },
    { body: 0x5a0a1a, engrave: 0xff7060, liquid: 0xc01030, liquidEmit: 0x800010 },
    { body: 0x2a1800, engrave: 0xf0b830, liquid: 0xf0a000, liquidEmit: 0xa06000 },
  ];
  ...
  const steamMat = new THREE.PointsMaterial({
    color: 0xf2e8d0, size: 0.04, transparent: true, opacity: 0.2,
    blending: THREE.AdditiveBlending, depthWrite: false,
  });
  ...
  const ingDefs = [
    { label: 'Maíz', color: 0xf0c040 },
    { label: 'Quinua', color: 0xe8d080 },
    { label: 'Canela', color: 0xa06030 },
    { label: 'Maracuyá', color: 0xff9020 },
    { label: 'Clavo', color: 0x804020 },
  ];
  ...
  scene.add(new THREE.Points(haloGeo, new THREE.PointsMaterial({ color: 0xc9a14a, size: 0.018, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending, depthWrite: false })));
  ...
  window.addEventListener('mousemove', e => { if (!isDragging) return; velX = (e.clientX - prevX) * 0.012; velY = (e.clientY - prevY) * 0.008; prevX = e.clientX; prevY = e.clientY; });
  window.addEventListener('touchmove', e => { if (!isDragging) return; velX = (e.touches[0].clientX - prevX) * 0.012; velY = (e.touches[0].clientY - prevY) * 0.008; prevX = e.touches[0].clientX; prevY = e.touches[0].clientY; }, { passive: true });
  canvas.addEventListener('wheel', e => { zoom = Math.max(3.5, Math.min(9, zoom + e.deltaY * 0.01)); e.preventDefault(); }, { passive: false });

  // ── VARIANT DATA ──
  const variantData = [
    { title: 'JORA', sub: 'La bebida de los Incas', desc: 'Maíz germinado, fermentado durante 72 horas en tinaja de arcilla ancestral. Notas terrosas, ligeramente ácida, con un final cálido y largo.', time: '72 horas', origin: 'Cusco', alc: '2–3%', temp: '8–10°C', price: 'S/ 18', ings: ['Maíz Jora', 'Agua Andina', 'Chancaca', 'Canela'] },
    { title: 'MORADA', sub: 'Espumante Contemporánea', desc: 'Maíz morado cusqueño, clavo, canela y piña de altura. Versión carbonatada y helada. Antioxidante natural con notas florales únicas.', time: '24 horas', origin: 'Lima', alc: '0.5%', temp: '4–6°C', price: 'S/ 15', ings: ['Maíz Morado', 'Clavo', 'Canela', 'Piña', 'Limón'] },
    { title: 'QUINUA', sub: 'Altiplano Sagrado', desc: 'Quinua blanca orgánica del altiplano peruano-boliviano. Ligera, nutritiva, con suaves notas nueces y un color dorado brillante.', time: '48 horas', origin: 'Altiplano', alc: '1.5%', temp: '6–8°C', price: 'S/ 20', ings: ['Quinua Blanca', 'Agua Mineral', 'Miel Andina', 'Cáscara Cítrica'] },
    { title: 'FRUTILLA', sub: 'Amazónica Vibrante', desc: 'Fresas silvestres, maracuyá amazónico y maíz amarillo. Fresca, vibrante, con un color rubí intenso. La favorita del verano.', time: '36 horas', origin: 'Amazónica', alc: '1%', temp: '4–6°C', price: 'S/ 16', ings: ['Frutillas Silvestres', 'Maracuyá', 'Maíz Amarillo', 'Azúcar de Caña'] },
    { title: 'CEREMONIAL', sub: 'Edición Limitada · 20/día', desc: 'Maíz sagrado cusqueño, fermentado por 7 días en vasija inca original. Servida a temperatura ritual. Solo 20 porciones diarias.', time: '7 días', origin: 'Cusco Inca', alc: '4–5%', temp: '12°C ritual', price: 'S/ 35', ings: ['Maíz Sagrado', 'Agua Manantial', 'Hierbas Rituales', 'Arcilla Inca'] },
  ];
  ...
  function changeVariant(idx) {
    ...
    $('#siTitle').textContent = v.title;
    $('#siSub').textContent = v.sub;
    $('#siDesc').textContent = v.desc;
    $('#siTime').textContent = v.time;
    $('#siOrigin').textContent = v.origin;
    $('#siAlc').textContent = v.alc;
    $('#siTemp').textContent = v.temp;
    $('#siPrice').textContent = v.price;
    $('#siIngs').innerHTML = v.ings.map(i => `<span class="si-ing">${i}</span>`).join('');
  ...
  $$('#shwVariants .shw-var').forEach(el => { el.addEventListener('click', () => changeVariant(parseInt(el.dataset.idx))); });
  $$('#shwDots .shw-dot').forEach((el, i) => { el.addEventListener('click', () => changeVariant(i)); });

  // ── ANIMATE ──
  const lOrigY = Array.from(liqGeo.attributes.position.array);
  let t = 0, autoRotate = true;
